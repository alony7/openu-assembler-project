    ...
    while (fgets(line, MAX_LINE_LENGTH, input_file) != NULL) {
        ...
        if (strcmp(parsed_line.main_operand, END_MACRO_DIRECTIVE) == 0) {
            if (!is_macro_line) {
                ...
                program_log(ERROR,line_number, "invalid end of macro", parsed_line.file_name, FALSE);
                return FALSE;
            }
            if (parsed_line.parameters_count != 0) {
                /* throw if the end macro directive has parameters */
                program_log(ERROR,line_number, join_strings(2, END_MACRO_DIRECTIVE, " takes no parameters"), parsed_line.file_name, TRUE);
                return FALSE;
            }
            is_macro_line = FALSE;
        ...
        /* if the line marks the start of a macro, create a new item for it and continue */
            ...
            if (parsed_line.parameters_count != 1) {
                ...
                program_log(ERROR,line_number, join_strings(2, START_MACRO_DIRECTIVE, " takes exactly 1 parameter"), parsed_line.file_name, TRUE);
                return FALSE;
            }
            if (get_instruction_type(parsed_line.parameters[0]) != UNKNOWN) {
                /* throw if the macro name is taken by an instruction */
                program_log(ERROR,line_number, join_strings(2, "macro name cannot be an instruction name: ", parsed_line.parameters[0]), parsed_line.file_name, TRUE);
                return FALSE;
            }
            if (is_macro_line) {
                /* throw if the start macro directive is inside another macro */
                program_log(ERROR,line_number, "nested macros are not allowed", parsed_line.file_name, FALSE);
                return FALSE;
            }
            if ((tmp_item = get_macro_item(table, parsed_line.parameters[0])) != NULL) {
                /* throw if the macro name is already taken */
                program_log(ERROR,line_number, join_strings(3, "macro '", parsed_line.parameters[0], "' already exists"), parsed_line.file_name, TRUE);
                return FALSE;
            }
