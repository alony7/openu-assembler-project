...
void parse_line(char *line, ParsedLine *parsed_line) {
    ...
    parsed_line->operand = malloc(strlen(operand) + 1);
...
FileOperands *parse_lines_from_file(FILE *file, char *file_name){
    ...
    ParsedLine *lines = malloc(BASE_LINE_BATCH_SIZE * sizeof(ParsedLine));
    ...
    while(fgets(line, MAX_LINE_LENGTH, file) != NULL){
        if(lines_count == lines_capacity){
            ...
            lines = realloc(lines, lines_capacity * sizeof(ParsedLine));
        }
