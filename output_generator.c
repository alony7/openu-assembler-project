...
Bool generate_output_files(char *object_filename, char *entry_filename, char *extern_filename,
    ...
    if(count_symbols_by_type(relocation_table,EXTERN)){
        ...
        for(i = 0;i<relocation_table->size;i++){
            /* TODO: fix this shit */
            if(relocation_table->symbols[i].type == EXTERN){
                fprintf(extern_file, "%s %d\n", relocation_table->symbols[i].name, relocation_table->symbols[i].address);
            }
        }
